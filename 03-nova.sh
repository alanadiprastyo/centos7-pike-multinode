...
openstack endpoint list | grep public | grep nova > /dev/null 2>&1 && echo "nova public endpoint already exists" || openstack endpoint create --region RegionOne compute public http://${CONT_HOSTNAME}:8774/v2.1
openstack endpoint list | grep internal | grep nova > /dev/null 2>&1 && echo "nova internal endpoint already exists" || openstack endpoint create --region RegionOne compute internal http://${CONT_HOSTNAME}:8774/v2.1
openstack endpoint list | grep admin | grep nova > /dev/null 2>&1 && echo "nova admin endpoint already exists" || openstack endpoint create --region RegionOne compute admin http://${CONT_HOSTNAME}:8774/v2.1
...
openstack endpoint list | grep public | grep placement > /dev/null 2>&1 && echo "placement public endpoint already exists" || openstack endpoint create --region RegionOne placement public http://${CONT_HOSTNAME}:8778
openstack endpoint list | grep internal | grep placement > /dev/null 2>&1 && echo "placement internal endpoint already exists" || openstack endpoint create --region RegionOne placement internal http://${CONT_HOSTNAME}:8778
openstack endpoint list | grep admin | grep placement > /dev/null 2>&1 && echo "placement admin endpoint already exists" || openstack endpoint create --region RegionOne placement admin http://${CONT_HOSTNAME}:8778
...
[api_database]
connection = mysql+pymysql://nova:${PASSWORD}@${CONT_HOSTNAME}/nova_api
[barbican]
[cache]
[cells]
[cinder]
...
[crypto]
[database]
connection = mysql+pymysql://nova:${PASSWORD}@${CONT_HOSTNAME}/nova
[devices]
[ephemeral_storage_encryption]
[filter_scheduler]
[glance]
api_servers = http://${CONT_HOSTNAME}:9292
[guestfs]
[healthcheck]
[hyperv]
[ironic]
...
[keystone]
[keystone_authtoken]
auth_url = http://${CONT_HOSTNAME}:5000/v3
...
[xvp]
_EOF_
cp ./tmp/nova.conf /etc/nova/nova.conf


[ ! -f /etc/httpd/conf.d/00-nova-placement-api.conf.orig ] && cp -v /etc/httpd/conf.d/00-nova-placement-api.conf /etc/httpd/conf.d/00-nova-placement-api.conf.orig
...
_EOF_
cp ./tmp/00-nova-placement-api.conf /etc/httpd/conf.d/00-nova-placement-api.conf

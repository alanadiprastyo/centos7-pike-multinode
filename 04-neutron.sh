...
openstack endpoint list | grep public | grep neutron > /dev/null 2>&1 && echo "neutron public endpoint already exists" || openstack endpoint create --region RegionOne network public http://${CONT_HOSTNAME}:9696
openstack endpoint list | grep internal | grep neutron > /dev/null 2>&1 && echo "neutron internal endpoint exists" || openstack endpoint create --region RegionOne network internal http://${CONT_HOSTNAME}:9696
openstack endpoint list | grep admin | grep neutron > /dev/null 2>&1 && echo "neutron admin endpoint already exists" || openstack endpoint create --region RegionOne neutron admin http://${CONT_HOSTNAME}:9696
...
[cors]
[database]
connection = mysql+pymysql://neutron:${PASSWORD}@${CONT_HOSTNAME}/neutron
[keystone_authtoken]
